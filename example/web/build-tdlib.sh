...
cd build/generate
cmake -j$nproc $TD_ROOT || exit 1
cd ../..

cd build/wasm
eval emconfigure cmake -DCMAKE_BUILD_TYPE=MinSizeRel -j$nproc $OPENSSL_OPTIONS $TD_ROOT || exit 1
cd ../..

cd build/asmjs
eval emconfigure cmake -DCMAKE_BUILD_TYPE=MinSizeRel -j$nproc $OPENSSL_OPTIONS -DASMJS=1 $TD_ROOT || exit 1
cd ../..

echo "Generating TDLib autogenerated source files..."
cmake --build build/generate --target prepare_cross_compiling -j$nproc || exit 1
echo "Building TDLib to WebAssembly..."
cmake --build build/wasm --target td_wasm -j$nproc || exit 1
echo "Building TDLib to asm.js..."
cmake --build build/asmjs --target td_asmjs -j$nproc || exit 1
